...
function showTab(category) {
  let tabContent = document.getElementById("product-list");
  tabContent.innerHTML = ""; // Clear previous content

  // Find the category data
  let categoryData = data.categories.find(
  ...
  if (categoryData) {
    categoryData.category_products.forEach((product) => {
      ...
      productElement.innerHTML = `
                <div class="product-tag">${product.badge_text}</div>
                <img class="image-height" src="${product.image}" alt="${product.title}" />
                ...
                <div class="product-info">
                        ...
                        <p class="actual-prize"> <del> ${product.compare_at_price}  </del> </p>
                        <p class="offer-color">50% Off</p>
                </div>
